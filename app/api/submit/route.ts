...
async function forwardToAppsScript(scriptUrl: string, payload: any) {
  ...
  const timeoutId = setTimeout(() => controller.abort(), 12000); // server-side timeout
...
export async function POST(req: Request) {
  try {
    ...
    const forwardPayload = {
      type: typeThai,
      name,
      phone: formatThaiPhone(phoneRaw), // formatted
    ...
    // ✅ Start forwarding, but DO NOT block the user experience
    forwardToAppsScript(scriptUrl, forwardPayload);
